...
const PipelineTab = {
  template: `
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
            <div>
              <div class="text-sm text-gray-600">Books Indexed</div>
              <div class="text-2xl font-bold text-blue-600">{{ stats.books_indexed }}</div>
            </div>
            <div class="text-4xl">📚</div>
          </div>
          ...
          <div class="flex items-center justify-between p-4 bg-purple-50 rounded-lg">
            <div>
              ...
              <div class="text-2xl font-bold text-purple-600">{{ stats.total_characters }}</div>
            </div>
            <div class="text-4xl">🎭</div>
          </div>
          ...
          <div class="flex items-center justify-between p-4 bg-orange-50 rounded-lg">
            <div>
              <div class="text-sm text-gray-600">Generated Stories</div>
              <div class="text-2xl font-bold text-orange-600">{{ stats.generated_stories }}</div>
            </div>
            <div class="text-4xl">✨</div>
          </div>
        </div>
        ...
        <button
          @click="refreshStats"
          class="w-full mt-4 bg-gray-100 text-gray-700 py-2 px-4 rounded-lg hover:bg-gray-200 transition-colors"
        >
          🔄 Refresh
        </button>
      </div>
      ...
      <!-- Book Ingestion -->
      <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-xl font-bold text-gray-800 mb-4">📥 Book Ingestion</h2>
        ...
        <button
          @click="ingestBooks"
          :disabled="selectedBooks.length === 0 || isIngesting"
          class="w-full bg-orange-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-orange-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <span v-if="!isIngesting">
            📥 Ingest Selected ({{ selectedBooks.length }})
          </span>
          <span v-else>
        ...
        <div v-if="isIngesting" class="mt-4">
      ...
      <div class="bg-white rounded-lg shadow-md p-6 lg:col-span-2">
        ...
        <div class="space-y-3">
          <div
            ...
            :key="activity.id"
            class="flex items-center space-x-4 p-3 border-l-4 rounded"
      ...
      <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-600">Redis Connection</span>
          ...
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-600">Vector Index</span>
            <span class="flex items-center text-sm">
              <span class="w-2 h-2 bg-green-500 rounded-full mr-2"></span>
              Ready
            </span>
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm font-medium">{{ stats.storage_mb }} MB</span>
          </div>
        </div>
      </div>
      ...
      <!-- API Usage -->
      <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-xl font-bold text-gray-800 mb-4">💰 API Usage (This Month)</h2>
        
        <div class="space-y-3">
          <div>
            <div class="flex items-center justify-between mb-1">
              <span class="text-sm text-gray-600">OpenAI Embeddings</span>
              <span class="text-sm font-medium">${{ usage.embeddings.toFixed(2) }}</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2">
          ...
          <div>
            <div class="flex items-center justify-between mb-1">
              <span class="text-sm text-gray-600">GPT-4 Story Gen</span>
              <span class="text-sm font-medium">${{ usage.gpt4.toFixed(2) }}</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2">
          ...
          <div>
            <div class="flex items-center justify-between mb-1">
              <span class="text-sm text-gray-600">Image Generation</span>
              <span class="text-sm font-medium">${{ usage.images.toFixed(2) }}</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2">
  ...
  computed: {
    selectedBooks() {
      return this.availableBooks.filter(b => b.selected && !b.indexed);
    },
    
    totalUsage() {
      return this.usage.embeddings + this.usage.gpt4 + this.usage.images;
    }
  },
  ...
  methods: {
    ...
    async ingestBooks() {
      ...
      try {
        const books = this.selectedBooks.map(b => ({
          title: b.title,
          url: b.url,
          category: b.category
        }));
        ...
        console.error('Error ingesting books:', error);
    ...
    activityColor(type) {
      const colors = {
        story: 'border-orange-400 bg-orange-50',
        book: 'border-blue-400 bg-blue-50',
        character: 'border-purple-400 bg-purple-50',
...
app.component('characters-tab', CharactersTab);
app.component('search-tab', SearchTab);
app.component('pipeline-tab', PipelineTab);
